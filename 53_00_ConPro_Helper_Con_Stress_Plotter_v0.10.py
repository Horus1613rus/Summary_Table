        ...
        for Res_File_Line in Res_File_Lines:
            ...
            if "Node :" in Res_File_Line:
                ...
                if not Node in Database[Wall]:
                    ...
                    if not "s1" in Database[Wall][Node]:
                        Database[Wall][Node]["s1"] = 0
                        Database[Wall][Node]["s2"] = 0
            if Read_Switch:
                if "steel" in Res_File_Line:
                    Read_Switch = False

            if Read_Switch:
                if "z+" in Res_File_Line:
                    if float(Res_File_Line.replace("z+","").split()[3]) < Database[Wall][Node]["s1"]:
                        Database[Wall][Node]["s1"] = float(Res_File_Line.replace("z+","").split()[3])
                    if float(Res_File_Line.replace("z+","").split()[4]) < Database[Wall][Node]["s2"]:
                        Database[Wall][Node]["s2"] = float(Res_File_Line.replace("z+","").split()[4])
                elif "z-" in Res_File_Line:
                    if float(Res_File_Line.replace("z-","").split()[3]) < Database[Wall][Node]["s1"]:
                        Database[Wall][Node]["s1"] = float(Res_File_Line.replace("z-","").split()[3])
                    if float(Res_File_Line.replace("z-","").split()[4]) < Database[Wall][Node]["s2"]:
                        Database[Wall][Node]["s2"] = float(Res_File_Line.replace("z-","").split()[4])
                else:
                    if float(Res_File_Line.split()[3]) < Database[Wall][Node]["s1"]:
                        Database[Wall][Node]["s1"] = float(Res_File_Line.split()[3])
                    if float(Res_File_Line.split()[4]) < Database[Wall][Node]["s2"]:
                        Database[Wall][Node]["s2"] = float(Res_File_Line.split()[4])

            if "layer          e1         e2       s1       s2      angle" in Res_File_Line:
                Read_Switch = True


if Txt_Plot:
    with open(os.getcwd() + '\\53_01_ConPro_Helper_Stress_Plotter_v'+Script_Ver+".txt", 'w' , encoding='utf-8') as Data_Out:
        Data_Out.write( "Wall\tNode\ts1\ts2\n" )
