...
Override_Value = {}
Override_Value["Row"] = 244
Override_Value["Column"] = 244
...
for Node in Raw_Coor:
    if X_Limit:
        if Raw_Coor[Node][Plot_Position.split()[0]] > float(X_Limit_Max) or Raw_Coor[Node][Plot_Position.split()[0]] < float(X_Limit_Min):
            if not Node in Remove_Nodes:
                Remove_Nodes.append(Node)
for Node in Raw_Coor:
    if Y_Limit:
        if Raw_Coor[Node][Plot_Position.split()[1]] > float(Y_Limit_Max) or Raw_Coor[Node][Plot_Position.split()[1]] < float(Y_Limit_Min):
            if not Node in Remove_Nodes:
                Remove_Nodes.append(Node)
...
for Axis in "Column Row".split():
    if Override_Align:
        Dummy = True
        #Cycle_Number = 
        #input(len(Raw_Plot_Values["Row"]))
    else:
        Cycle_Number = len(Raw_Plot_Values[Axis])
    Plot_Values[Axis] = []
    if Axis == "Column":
        i = 0
    else:
