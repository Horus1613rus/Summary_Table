...
Worksheet_Co = Workbook.add_sheet('Construction Cracks',cell_overwrite_ok=True)
...
i=0
for i in range(len(Co_Minimaxes)):
    Worksheet_Co.write(0,(i*3)+1,"Crack")
    Worksheet_Co.col((i*3)+1).width = 2560
    Worksheet_Co.write(0,(i*3)+2,"Minimax"+str(Co_Minimaxes[i]))
    Worksheet_Co.col((i*3)+2).width = 3840
    Worksheet_Co.write(0,(i*3)+3,"LC")
    Worksheet_Co.col((i*3)+3).width = 5120
...
for DG in Database:
    for Minimax in Co_Minimaxes:
            Row = DG_List.index(DG) + 1 
            Column = (3 * Co_Minimaxes.index(Minimax)) + 1
            Worksheet_Co.write(Row,0,DG)
            Worksheet_Co.write(Row,Column,Database[DG][Minimax]["Crack_Value"])
            Worksheet_Co.write(Row,Column+1,str(Database[DG][Minimax]["Crack_Element"]) + " " + str(Database[DG][Minimax]["Crack_Node"]))
            Worksheet_Co.write(Row,Column+2,str(Database[DG][Minimax]["Crack_LC"]) + str(Database[DG][Minimax]["Thermal"]))
            Worksheet_Co.write(Row,10,xlwt.Formula("MAX(B" + str(Row+1) + ",E" + str(Row+1) + ",H" + str(Row+1) + ")"))
